...
{{/* Validate at least one master node is configured when the cluster has active nodes */}}
{{- define "elasticsearch.validateValues.master.replicas" -}}
{{- $hasNodes := or (include "elasticsearch.coordinating.enabled" .) (include "elasticsearch.data.enabled" .) (include "elasticsearch.ingest.enabled" .) -}}
{{- if and (not (include "elasticsearch.master.enabled" .)) $hasNodes -}}
elasticsearch: master.replicas
    Elasticsearch needs at least one master-elegible node when data, coordinating, or ingest nodes are enabled.
    To stop the cluster, set master.replicaCount, data.replicaCount, coordinating.replicaCount, and ingest.replicaCount to 0.
{{- end -}}
{{- end -}}
